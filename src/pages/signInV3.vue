...
const penpalMethods = {
  ...
  triggerSocialLogin: (
    kind: Exclude<SocialLoginType, SocialLoginType.passwordless>
  ) => handleSocialLoginRequest(kind),
...
const handleSocialLoginRequest = async (
  k: Exclude<SocialLoginType, SocialLoginType.passwordless>
) => {
  const provider = await getAuthProvider(appId as string)
  provider.loginWithSocial(k)
}
